...
import { OnFinishedConsultationUseCase } from './on-finished-consultation-use-case'
...
let sut: OnFinishedConsultationUseCase
...
describe('Create scheduling use case', () => {
  beforeEach(() => {
    ...
    sut = new OnFinishedConsultationUseCase(schedulingRepository)
  ...
  it('should be able to create a scheduling', async () => {
    ...
    const createdScheduling = await schedulingRepository.create({
      professionalPersonId: professional.person_id,
    ...
    const { scheduling } = await sut.execute({
      schedulingId: createdScheduling.id,
    })

    expect(scheduling.onFinishedConsultation).toEqual(true)
  })
})
