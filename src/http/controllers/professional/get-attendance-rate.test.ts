...
describe('Get attendance rate by professional id', () => {
  it('should be able to get the attendance rate for a professional', async () => {
    ...
    const reply = await request(app.server).get(
      `/professionals/attendance-rate/${professional.person_id}?startDay=2025-10-10&endDay=2025-10-14`
    )

    expect(reply.statusCode).toEqual(200)
    expect(reply.body.attendanceRate).toEqual(50)
  })
})
