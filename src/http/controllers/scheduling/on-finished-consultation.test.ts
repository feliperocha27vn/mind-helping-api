...
describe('Mark consultation as finished', () => {
  it('should be able to mark consultation as finished', async () => {
    ...
    const reply = await request(app.server).patch(
      `/schedulings/finished-consultation/${scheduling.id}`
    )

    expect(reply.status).toBe(204)

    const updatedScheduling = await prisma.scheduling.findUniqueOrThrow({
      where: {
        id: scheduling.id,
      },
    })

    expect(updatedScheduling.onFinishedConsultation).toEqual(true)
  })
})
